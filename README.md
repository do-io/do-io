<img src="https://avatars2.githubusercontent.com/u/235454?s=460&u=6f0f5bec3e1f5d3d0656d896b8de9e5fbab40680&v=4" title="developer & designer" align="right" width="150" />

# Hi there, I'm Darren! 👋

I'm a **Software Developer & Web Architect** focused on building clean, scalable, and high-performing applications for the modern web. 

---

### 🚀 What I Do
* 🛠️ **Back-End:** Specializing in modern PHP, Laravel ecosystems, and Python.
* 🎨 **Front-End:** Crafting reactive and intuitive user interfaces with Vue.js.
* 📢 **Advocacy:** Championing the modern capabilities, speed, and power of PHP.

### 🌐 My Projects
I am the creator and maintainer of **[dophp.dev](https://dophp.dev)**, a platform dedicated to modern PHP development, best practices, and ecosystem advocacy. Check it out to see what the language can do today!

---

### 📊 Tech Stack & Tools

* **Languages:** PHP, JavaScript, Python, SQL
* **Frameworks:** Laravel, Vue.js
* **Knowledge Management:** Obsidian 🪵
* **Ecosystem:** Apple / macOS / iOS

---

### 🌲 Beyond the Code
When I'm not pushing commits, you can usually find me:
* Camping and exploring the mountains in a travel trailer with my family.
* Tackling the daily crossword puzzle.
* Checking out classic hip-hop history and wild-style graffiti art.
* Cheering on the Steelers, Seahawks, and Cubs.

---

### 📫 Connect with Me
* **Website:** [dophp.dev](https://dophp.dev)
* **LinkedIn:** [Darren Odden](https://linkedin.com/in/darren.odden)

<!--
**do-io/do-io** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
